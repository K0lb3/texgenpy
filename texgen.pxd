...
cdef extern from "texgenpack.h":
    ...
    cdef double  compare_block_4x4_signed_8_bit_components_with_16_bit(unsigned int *image_buffer,
    ...
    cdef size_t buf_read(void*dst, size_t i, size_t size, Buffer*src)
    cdef void load_image_from_memory(Buffer *buf, int filetype, Image *image)
    cdef void load_pkm_buffer(Buffer *f, Texture *texture)
    cdef int load_ktx_buffer(Buffer *f, int max_mipmaps, Texture *texture)
    cdef int load_dds_buffer(Buffer *f, int max_mipmaps, Texture *texture)
    cdef int load_astc_buffer(Buffer *f, Texture *texture)
